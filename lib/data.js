...
export async function getAnalyticsSummary(restaurantId) {
    ...
    const orders = await Order.find({ restaurantId: objectId }).lean();
    ...
    for (const order of orders) {
        ...
        const orderRevenue = typeof order.subtotal === "number" ? order.subtotal : order.totalAmount;
    ...
    return {
        sales: {
            daily: orders
                .filter((order) => new Date(order.createdAt) >= startOfDay(now))
                .reduce((sum, order) => sum + (typeof order.subtotal === "number" ? order.subtotal : order.totalAmount), 0),
            weekly: orders
                .filter((order) => new Date(order.createdAt) >= startOfWeek(now))
                .reduce((sum, order) => sum + (typeof order.subtotal === "number" ? order.subtotal : order.totalAmount), 0),
            monthly: orders
                .filter((order) => new Date(order.createdAt) >= startOfMonth(now))
                .reduce((sum, order) => sum + (typeof order.subtotal === "number" ? order.subtotal : order.totalAmount), 0)
        },
